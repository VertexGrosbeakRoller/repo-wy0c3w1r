...
public enum Javelin implements ClientModInitializer {
   ...
   private static final String MOD_ID = "CopperHead".toLowerCase();
   ...
   private boolean initialized = false;
    ...
    public static int multAlpha(int baseColor, float v) {
        return 0;
    }
   ...
   public void init() {
      ...
      try {
         DIRECTORY = new File(MinecraftClient.getInstance().runDirectory, "CopperHead");
         if (!DIRECTORY.exists()) {
            DIRECTORY.mkdirs();
         }
         ...
         e.printStackTrace();
         throw new RuntimeException("CopperHead initialization failed", e);
      }
   }
