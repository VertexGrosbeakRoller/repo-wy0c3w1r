package tech.javelin.base.waypoint;
   ...
   public void onHUD(EventHudRender e) {
      if (mc.player != null) {
         ...
         if (this.activeWaypoint != null) {
            ...
            e.getContext().getMatrices().translate(-x, -y, 0.0F);
            e.getContext().drawTexture(Javelin.id("icons/arrow.png"), x - size / 2.0F, y - size / 2.0F, size, size, Javelin.getInstance().getThemeManager().getCurrentTheme().getColor());
            e.getContext().getMatrices().pop();
         }

         if (this.activePlayerWaypoint != null) {
            x = (float)mc.getWindow().getScaledWidth() / 2.0F;
            y = (float)mc.getWindow().getScaledHeight() / 1.7F;  // Еще ниже на экране
            size = 16.0F;
            e.getContext().getMatrices().push();
            x2 = this.activeWaypoint.getX() - mc.player.getX();
            z2 = this.activeWaypoint.getZ() - mc.player.getZ();
            ...
            e.getContext().getMatrices().multiply(RotationAxis.POSITIVE_Z.rotationDegrees(yaw));
            e.getContext().getMatrices().translate(-x, -y, 0.0F);
            e.getContext().drawTexture(Javelin.id("icons/arrow.png"), x - size / 2.0F, y - size / 2.0F, size, size, new ColorRGBA(255, 32, 32));
            e.getContext().getMatrices().pop();
         }
