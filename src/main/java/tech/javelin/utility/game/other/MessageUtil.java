...
public final class MessageUtil implements IMinecraft {
   ...
   public static void displayMessage(MessageUtil.LogLevel level, Object message) {
      // Проверяем UnHook - скрываем сообщения чита
      if (tech.javelin.client.modules.impl.misc.UnHook.INSTANCE.shouldHideMessages()) {
         return;
      }
      Text icon = createGradientText("CopperHead", Javelin.getInstance().getThemeManager().getCurrentTheme().getColor(), Javelin.getInstance().getThemeManager().getCurrentTheme().getSecondColor());
