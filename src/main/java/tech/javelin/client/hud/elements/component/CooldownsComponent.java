...
public class CooldownsComponent extends DraggableHudElement {
   ...
   public CooldownsComponent(String name, float initialX, float initialY, float windowWidth, float windowHeight, float offsetX, float offsetY, DraggableHudElement.Align align) {
   ...
   public void render(CustomDrawContext ctx) {
      ...
      for (Item item : vanillaItems) {
         ...
         float progress = mc.player.getItemCooldownManager().getCooldownProgress(stack, mc.getRenderTickCounter().getTickDelta(true));
      ...
      ctx.drawText(Fonts.SEMIBOLD.getFont(7.5F), "Cooldowns", posX + 18.5F, posY + 4.75F, (new ColorRGBA(-1)).withAlpha(255.0F * this.alpha.getValue()));
      ...
      for (CooldownEntry entry : cooldowns) {
         ...
         height += (11.0F + 2.0F) * anim.getValue(); // Увеличен отступ с 0.5F до 2.0F

         drawBlurBackground(ctx, posX, elementY, this.widthAnimation.getValue(), 11.0F, theme, elementAlpha);

         ctx.drawText(Fonts.SEMIBOLD.getFont(7.0F), entry.name, posX + 5.0F, elementY + 3.25F, (new ColorRGBA(-1)).withAlpha(elementAlpha * 255.0F));
         ...
         ctx.drawText(Fonts.SEMIBOLD.getFont(6.75F), timeText, timerX, elementY + 3.25F, (new ColorRGBA(-1)).withAlpha(elementAlpha * 255.0F));
         ...
         ctx.pushMatrix();
         ctx.getMatrices().translate(posX + this.widthAnimation.getValue() - 14.0F, elementY + 1.5F, 0.0D);
         ...
         posY += (11.0F + 2.0F) * anim.getValue(); // Увеличен отступ с 0.5F до 2.0F
      }
   ...
   private String getItemName(Item item) {
      ...
      if (item == Items.CHORUS_FRUIT) return "Плод хоруса";
      if (item == Items.SHIELD) return "Щит";
      if (item == Items.CROSSBOW) return "Арбалет";
      if (item == Items.TRIDENT) return "Трезубец";
      if (item == Items.GOAT_HORN) return "Козий рог";
      if (item == Items.GOLDEN_APPLE) return "Золотое яблоко";
      if (item == Items.ENCHANTED_GOLDEN_APPLE) return "Зачарованное яблоко";
   ...
   private String getCustomItemName(ItemStack stack) {
      ...
      if (customData.getNbt().contains("pyrotechnic-item")) {
         String pyroType = customData.getNbt().getString("pyrotechnic-item");
         if ("ALTERNATIVE_TRAP".equals(pyroType)) return "Трапка";
         if ("EXPLOSIVE_TRAP".equals(pyroType)) return "Взрывная Трапка";
         if ("STUN_STAR".equals(pyroType)) return "Стан";
      }

      if (customData.getNbt().contains("kringeItems")) {
         String kringeType = customData.getNbt().getString("kringeItems");
         if ("ExplosiveStuff".equals(kringeType)) return "Взрывная штучка";
      }

      if (customData.getNbt().contains("desorientation")) {
         return "Дезориентация";
      }

      if (customData.getNbt().contains("stratum")) {
         return "Пласт";
      }

      if (customData.getNbt().contains("trap")) {
         return "Трапка";
      }

      if (customData.getNbt().contains("sheerdust")) {
         return "Явная пыль";
      }

      if (customData.getNbt().contains("godsaura")) {
         return "Божья аура";
      }

      if (customData.getNbt().contains("sphereEffect")) {
         String sphereType = customData.getNbt().getString("sphereEffect");
         if (sphereType.contains("speed")) return "Сфера скорости";
         if (sphereType.contains("strength")) return "Сфера силы";
         if (sphereType.contains("regeneration")) return "Сфера регенерации";
         if (sphereType.contains("resistance")) return "Сфера сопротивления";
         if (sphereType.contains("fire_resistance")) return "Сфера огнестойкости";
         if (sphereType.contains("invisibility")) return "Сфера невидимости";
         if (sphereType.contains("night_vision")) return "Сфера ночного зрения";
         if (sphereType.contains("water_breathing")) return "Сфера подводного дыхания";
         if (sphereType.contains("jump_boost")) return "Сфера прыгучести";
         if (sphereType.contains("absorption")) return "Сфера поглощения";
         if (sphereType.contains("health_boost")) return "Сфера здоровья";
         return "Сфера";
      }
   ...
   private String formatTime(float seconds) {
      ...
      return String.format("%dс", (int) Math.ceil(seconds));
   }
