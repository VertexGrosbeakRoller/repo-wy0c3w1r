package tech.javelin.client.screens.menu;
...
import tech.javelin.client.screens.menu.components.UserComponent;
import tech.javelin.utility.game.other.ReplaceUtil;
...
public class MenuScreen extends Screen implements IClient {
   ...
   protected final Animation slideAnimation;
   ...
   public MenuScreen() {
      ...
      this.slideAnimation = new Animation(400L, Easing.CUBIC_OUT);
      ...
      Iterator var5 = Javelin.getInstance().getModuleManager().getModules().iterator();
      ...
      if (!this.needToClose) {
         this.savedRunnable = new Runnable() {
            public void run() {
               ...
               int centerX = screenWidth / 2 - 176;
               int y = screenHeight / 2 - 99;
               // Slide animation: opening from LEFT, closing to RIGHT
               float slideOffset;
               if (MenuScreen.this.needToClose) {
                  // Closing: slide from center to RIGHT (off-screen)
                  slideOffset = (1.0F - MenuScreen.this.slideAnimation.getValue()) * (screenWidth - centerX + 100);
               } else {
                  // Opening: slide from LEFT (off-screen) to center
                  slideOffset = (1.0F - MenuScreen.this.slideAnimation.getValue()) * -(centerX + 400);
               }
               int x = centerX + (int)slideOffset;
               float lineX = 74.0F;
               // No initial animation effect - just slide
               MenuScreen.this.openAnimationMetanoise.setEasing(Easing.LINEAR);
               MenuScreen.this.openAnimationMetanoise.setDuration(0L);
               ...
               drawContext.drawText(Fonts.REGULAR.getFont(7.0F), "CopperHead", (float)(x + 92), (float)(y + 13), new ColorRGBA(67, 67, 67, MenuScreen.this.openAnimation.getValue() * 255.0F));
               ...
               drawContext.drawText(Fonts.REGULAR.getFont(7.0F), "CopperHead", (float)(x + 22), (float)(y + 10), new ColorRGBA(247, 251, 252, MenuScreen.this.openAnimation.getValue() * 255.0F));
               ...
               for(Iterator var9 = MenuScreen.this.categories.entrySet().iterator(); var9.hasNext(); categoryY += 19.0F) {
                  ...
                  // Increased transparency for selected category
                  float selectedAlpha = MenuScreen.this.currentCategory == category.getKey() ? 180.0F : 100.0F;
                  start = ColorUtil.interpolate(ColorRGBA.GRAY.withAlpha(MenuScreen.this.openAnimation.getValue() * selectedAlpha), (new ColorRGBA(-1)).withAlpha(MenuScreen.this.openAnimation.getValue() * 255.0F), ((Animation)category.getValue()).getValue()).getRGB();
                  end = ColorUtil.interpolate(ColorRGBA.GRAY.withAlpha(MenuScreen.this.openAnimation.getValue() * selectedAlpha), theme.getColor().withAlpha(MenuScreen.this.openAnimation.getValue() * 255.0F), ((Animation)category.getValue()).getValue()).getRGB();
               ...
               if (MenuScreen.this.currentCategory == Category.THEMES) {
                  moduleX = (float)(x + 83);
                  moduleY = (float)(y + 21) + MenuScreen.this.scroll.getValue();
                  int i = 0;
                  Iterator var48 = Javelin.getInstance().getThemeManager().getThemes().iterator();
                  ...
                  while(var48.hasNext()) {
                     ...
                     DrawUtil.drawRoundedBorder(drawContext.getMatrices(), moduleX, moduleY, 129.0F, 15.0F, -1.0F, BorderRadius.all(2.0F), new ColorRGBA(211, 211, 211, Javelin.getInstance().getThemeManager().getCurrentTheme() == theme1 ? (float)((int)(255.0F * MenuScreen.this.animation.getValue())) * MenuScreen.this.openAnimation.getValue() : (float)((int)(130.0F * MenuScreen.this.animation.getValue())) * MenuScreen.this.openAnimation.getValue()));
                     drawContext.drawText(Fonts.REGULAR.getFont(8.0F), theme1.getName(), moduleX + 4.0F, moduleY + 5.0F, (new ColorRGBA(222, 222, 222)).withAlpha(Javelin.getInstance().getThemeManager().getCurrentTheme() == theme1 ? (float)((int)(255.0F * MenuScreen.this.animation.getValue())) * MenuScreen.this.openAnimation.getValue() : (float)((int)(130.0F * MenuScreen.this.animation.getValue())) * MenuScreen.this.openAnimation.getValue()));
                     theme1.drawTheme(drawContext.getMatrices(), Javelin.getInstance().getThemeManager().getCurrentTheme() == theme1 ? (double)((float)((int)(255.0F * MenuScreen.this.animation.getValue())) * MenuScreen.this.openAnimation.getValue()) : (double)((float)((int)(130.0F * MenuScreen.this.animation.getValue())) * MenuScreen.this.openAnimation.getValue()));
                     moduleX += 133.0F;
                     ++i;
                     theme1.setX(moduleX);
                     theme1.setY(moduleY);
                  ...
                  MenuScreen.this.columns = new Vec2f(moduleY + 18.0F, moduleY + 18.0F);
                  StencilUtil.pop();
               } else {
                  ...
                  Iterator var17 = Javelin.getInstance().getModuleManager().getModules().iterator();
                        ...
                        do {
                           if (!var21.hasNext()) {
                              ...
                              if (moduleY + baseHeight > (float)y - settingHeight + 40.0F) {
                                 DrawUtil.drawRoundedBorder(drawContext.getMatrices(), moduleX, moduleY, 129.0F, totalHeight, -1.0F, BorderRadius.all(2.0F), color);
                                 if (module.getAnimation().getValue() > 0.0F) {
                                    DrawUtil.drawRoundedRect(drawContext.getMatrices(), moduleX, moduleY, 129.0F, totalHeight, BorderRadius.all(1.5F), (new ColorRGBA(22, 22, 22)).withAlpha(module.getAnimation().getValue() * 130.0F * MenuScreen.this.animation.getValue() * MenuScreen.this.openAnimation.getValue()));
                                 }
                                 ...
                                 // ArrayList-style per-character flowing gradient for module name
                                 String moduleName = module.getName();
                                 float textX = moduleX + (float)(module.getInfo().description().isEmpty() ? 5 : 13);
                                 float charX = textX;
                                 double tMs = System.currentTimeMillis() * 0.008;
                                 int segments = Math.max(6, moduleName.length()); // More segments for clearer gradient
                                 for (int ci = 0; ci < moduleName.length(); ci++) {
                                    float phase = (float)((tMs + ci * 0.8) % (Math.PI * 2));
                                    float wave = (float)(0.5 + 0.5 * Math.sin(phase));
                                    float wave2 = (float)(0.5 + 0.5 * Math.sin(phase * 1.3 + 1.0));
                                    float mix = MathHelper.clamp(wave * 0.6f + wave2 * 0.4f, 0f, 1f);
                                    ColorRGBA charColor = ColorUtil.interpolate(theme.getColor(), theme.getSecondColor(), mix)
                                        .withAlpha((int)(255.0F * MenuScreen.this.animation.getValue() * MenuScreen.this.openAnimation.getValue()));
                                    String ch = moduleName.substring(ci, ci + 1);
                                    drawContext.drawText(Fonts.REGULAR.getFont(7.0F), ch, charX, moduleY + 6.5F, module.getAnimation().getValue() > 0.0F ? charColor : colorTextModule);
                                    charX += Fonts.REGULAR.getWidth(ch, 7.0F);
                                 }
      ...
      if (this.currentSettingBinding != null) {
         ...
         for(i = 0; i < var10; ++i) {
            ...
            if (MathUtil.isHovered(mouseX, mouseY, (double)(x + 6), (double)(categoryY - 10.0F), 55.0D, 18.0D) && this.currentCategory != category1) {
               ...
               var13 = Javelin.getInstance().getModuleManager().getModules().iterator();
         ...
         if (this.currentCategory == Category.THEMES) {
            moduleX = (float)(x + 83);
            moduleY = (float)(y + 21) + this.scroll.getValue();
            i = 0;
            Iterator var33 = Javelin.getInstance().getThemeManager().getThemes().iterator();
            ...
            while(var33.hasNext()) {
               ...
               if (MathUtil.isHovered(mouseX, mouseY, (double)moduleX, (double)moduleY, 129.0D, 15.0D) && Javelin.getInstance().getThemeManager().getCurrentTheme() != theme1) {
                  theme1.getAnimation().setValue(0.0F);
                  theme1.startAnimation(Javelin.getInstance().getThemeManager().getCurrentTheme().getColor1(), Javelin.getInstance().getThemeManager().getCurrentTheme().getColor2());
                  Javelin.getInstance().getThemeManager().setCurrentTheme(theme1);
               }
   ...
   private void updateAnimations() {
      ...
      // Slide in from LEFT when opening, slide out to RIGHT when closing
      if (this.needToClose) {
         this.slideAnimation.update(0.0F); // Slide out to right (value goes to 0)
      } else {
         this.slideAnimation.update(1.0F); // Slide in from left (value goes to 1)
      }
   }
