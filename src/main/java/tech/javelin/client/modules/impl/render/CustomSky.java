...
public class CustomSky extends Module implements IMinecraft {
    ...
    private static final GlProgram WATER_SHADER = new GlProgram(Javelin.id("skyshader/water"), VertexFormats.POSITION);
    private static final GlProgram CAUSTIC_SHADER = new GlProgram(Javelin.id("skyshader/caustic"), VertexFormats.POSITION);
    private static final GlProgram PLASMA_SHADER = new GlProgram(Javelin.id("skyshader/sky_plasma"), VertexFormats.POSITION);
    ...
    public void renderSkyShader() {
        ...
        // Ensure valid framebuffer dimensions
        if (fw <= 0 || fh <= 0) {
            fw = 1920;
            fh = 1080;
        }
        ...
        ColorRGBA secondColor = Javelin.getInstance().getThemeManager().getCurrentTheme().getSecondColor();
