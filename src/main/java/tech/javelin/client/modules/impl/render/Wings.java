...
@ModuleAnnotation(
   ...
   description = "Полупрозрачные крылья за спиной"
)
public final class Wings extends Module {
   ...
   private final BooleanSetting self    = new BooleanSetting("Себе",    true);
   private final BooleanSetting players = new BooleanSetting("Игрокам", false);
   ...
   private void onRender3D(EventRender3D event) {
      ...
      if (self.isEnabled() && !isFirstPerson && mc.player.isAlive() && !hasElytra(mc.player)) {
   ...
   private int resolveBaseColor() {
      return Javelin.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB();
   }

   private int resolveGlowColor(int base) {
      int white = makeColor(255, 255, 255, 255);
      return interpolateColor(base, white, 0.28f);
   }

   private int resolveCoreColor(int base) {
      int white = makeColor(255, 255, 255, 255);
      return interpolateColor(base, white, 0.55f);
   }
   ...
   private WingPose resolvePose(PlayerEntity player, float tickDelta) {
      ...
      if (player.isGliding()) {
         ...
         return new WingPose(0.34f, 0.46f, 0f, 0f, pitchRotation, 0f,
               0.76f, 0.92f, 0.10f, 0.58f, 0.05f, 0f, 0.06f, -5f, -2f, 0.13f);
      }
      ...
      if (player.isSneaking()) {
         return new WingPose(0f, 0f, 0.96f, 0.10f, 18f, 0f,
               1f, 1f, 0.18f, 4.5f, 0.06f, 0f, 0.02f, -11f, -4f, 0.12f);
      }
