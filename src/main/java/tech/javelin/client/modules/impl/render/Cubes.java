...
@ModuleAnnotation(
        ...
        description = "3D Кубы по миру"
)
public class Cubes extends Module {

    public static Cubes INSTANCE = new Cubes();

    private static final Identifier GLOW_TEX = Identifier.of("wyvern", "icons/glow.png");
    ...
    private final ModeSetting animation = new ModeSetting("Анимация", "Разлет", "Падение");
    private final ModeSetting shape = new ModeSetting("Форма", "Кубы", "Треугольники");
    ...
    private void applyHitImpulseFromCrosshair(Camera camera) {
        ...
        if (best != null) {
            double force = 0.08 * speed.getCurrent();
            best.vx += dirX * force;
    ...
    private void updateCubes() {
        ...
        float spd = PARTICLE_SPEED * speed.getCurrent();
    ...
    private void renderCubes(EventRender3D e) {
        ...
        float s = PARTICLE_SIZE * size.getCurrent();
        float glow = GLOW_INTENSITY;

        int baseRGB = JAVELIN.getInstance().getThemeManager().getClientColor(0).getRGB();
    ...
    private CubeParticle spawnCube() {
        ...
        float speedMult = speed.getCurrent();
        float vx, vy, vz;
